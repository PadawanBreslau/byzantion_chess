...
require_relative 'move_info_parser'
...
class Game
  ...
  def en_passant?(piece, board, info)
    piece.kind_of?(ByzantionChess::Pawn) && !board.piece_from(info[:field]) && info[:take]
  end
  ...
  def is_white?(info)
    ByzantionChess::WHITE == info[:piece_color]
  end
  ...
  def create_castle(move, board)
    ...
    ByzantionChess::Castle.new(start_field, destination, info[:piece_color],last_move_number)
  end
